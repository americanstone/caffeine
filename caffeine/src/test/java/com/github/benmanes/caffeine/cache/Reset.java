...
public final class Reset {
  ...
  public static void destroy(Cache<?, ?> cache) {
    ...
    if (local instanceof UnboundedLocalCache<?, ?>) {
      ...
      BoundedLocalCache<?, ?> bounded = (BoundedLocalCache<?, ?>) local;
  ...
  private static void destroyTimerWheel(BoundedLocalCache<?, ?> bounded) {
    for (int i = 0; i < bounded.timerWheel().wheel.length; i++) {
      for (Node<?, ?> sentinel : bounded.timerWheel().wheel[i]) {
        sentinel.setPreviousInVariableOrder(null);
        sentinel.setNextInVariableOrder(null);
      }
    }
  }
}
